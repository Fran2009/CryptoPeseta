...
constexpr std::chrono::microseconds CRYPTOPESETASECOND = std::chrono::microseconds{1};
...
void BuildSingleTest(Scenario& scenario, int config)
{
    ...
    if (immediate) {
        ...
        scenario.AdvanceTime(delay - CRYPTOPESETASECOND);
        scenario.Check(peer, {}, 1, 0, 0, "s3");
        scenario.AdvanceTime(CRYPTOPESETASECOND);
    ...
    if (config >> 3) { // We'll request the transaction
        ...
        if ((config >> 3) == 1) { // The request will time out
            scenario.AdvanceTime(expiry - CRYPTOPESETASECOND);
...
void BuildBigPriorityTest(Scenario& scenario, int peers)
{
    ...
    for (int i = peers - 1; i >= 0; --i) {
        scenario.AdvanceTime(reqtimes[request_order[i]] - scenario.Now() - CRYPTOPESETASECOND);
...
void BuildRequestOrderTest(Scenario& scenario, int config)
{
    ...
    scenario.AdvanceTime(reqtime2 - CRYPTOPESETASECOND - scenario.Now());
    scenario.Check(peer, {}, 2, 0, 0, "o1");
    scenario.AdvanceTime(CRYPTOPESETASECOND);
    scenario.Check(peer, {gtxid2}, 2, 0, 0, "o2");
    scenario.AdvanceTime(reqtime1 - CRYPTOPESETASECOND - scenario.Now());
...
void BuildTimeBackwardsTest(Scenario& scenario)
{
    ...
    scenario.Check(peer2, {}, 1, 0, 0, "r3", -CRYPTOPESETASECOND);
    ...
    scenario.Check(peer1, {}, 0, 0, 1, "r11", -CRYPTOPESETASECOND); // Going back does not unexpire.
    scenario.Check(peer2, {gtxid}, 1, 0, 0, "r12", -CRYPTOPESETASECOND);
