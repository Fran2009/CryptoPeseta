...
static void cryptopesetaTask(CScheduler& s, std::mutex& mutex, int& counter, int delta, std::chrono::system_clock::time_point rescheduleTime)
{
    {
    ...
    if (rescheduleTime != noTime) {
        CScheduler::Function f = std::bind(&cryptopesetaTask, std::ref(s), std::ref(mutex), std::ref(counter), -delta + 1, noTime);
...
BOOST_AUTO_TEST_CASE(manythreads)
{
    ...
    size_t nTasks = cryptopesetaTasks.getQueueInfo(first, last);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        CScheduler::Function f = std::bind(&cryptopesetaTask, std::ref(cryptopesetaTasks),
                                             std::ref(counterMutex[whichCounter]), std::ref(counter[whichCounter]),
                                             randomDelta(rng), tReschedule);
        cryptopesetaTasks.schedule(f, t);
    }
    nTasks = cryptopesetaTasks.getQueueInfo(first, last);
    BOOST_CHECK(nTasks == 100);
    BOOST_CHECK(first < last);
    BOOST_CHECK(last > now);

    // As soon as these are created they will start running and servicing the queue
    std::vector<std::thread> cryptopesetaThreads;
    for (int i = 0; i < 5; i++)
        cryptopesetaThreads.emplace_back(std::bind(&CScheduler::serviceQueue, &cryptopesetaTasks));
    ...
    cryptopesetaTasks.StopWhenDrained();
    // wait until all the threads are done
    for (auto& thread: cryptopesetaThreads) {
