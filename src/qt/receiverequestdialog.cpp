...
#include <config/cryptopeseta-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatCryptoPesetaURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(CryptoPesetaUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatCryptoPesetaURI(info));
}
