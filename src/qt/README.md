This directory contains the source code for the CryptoPeseta graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### cryptopesetagui.(h/cpp)

- Represents the main window of the CryptoPeseta UI.
...
- ClientModel is used by the main application `cryptopesetagui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a cryptopeseta:... link from a browser).
...
* UI elements like CryptoPesetaAmountField, which inherit from QWidget.
* `cryptopesetastrings.cpp`: automatically generated
* `cryptopesetaunits.(h/cpp)`: CPTS / mCPTS / etc. handling
* `callback.h`
...
**Note:** Do not change `local/cryptopeseta_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `cryptopeseta-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `cryptopeseta-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `cryptopeseta-qt`
9. You're all set! Start developing, building, and debugging the CryptoPeseta GUI
