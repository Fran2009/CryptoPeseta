...
#include <qt/cryptopesetaaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CryptoPeseta address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new CryptoPesetaAddressEntryValidator(parent));
    widget->setCheckValidator(new CryptoPesetaAddressCheckValidator(parent));
}
...
bool parseCryptoPesetaURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no cryptopeseta: URI
    if(!uri.isValid() || uri.scheme() != QString("cryptopeseta"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!CryptoPesetaUnits::parse(CryptoPesetaUnits::CPTS, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseCryptoPesetaURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseCryptoPesetaURI(uriInstance, out);
}

QString formatCryptoPesetaURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("cryptopeseta:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(CryptoPesetaUnits::format(CryptoPesetaUnits::CPTS, info.amount, false, CryptoPesetaUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openCryptoPesetaConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", CRYPTOPESETA_CONF_FILENAME));
    ...
    /* Open cryptopeseta.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CryptoPeseta.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "CryptoPeseta (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("CryptoPeseta (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for CryptoPeseta*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "cryptopeseta.desktop";
    return GetAutostartDir() / strprintf("cryptopeseta-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a cryptopeseta.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=CryptoPeseta\n";
        else
            optionFile << strprintf("Name=CryptoPeseta (%s)\n", chain);
