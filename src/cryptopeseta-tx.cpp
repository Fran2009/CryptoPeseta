...
static void SetupCryptoPesetaTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " cryptopeseta-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  cryptopeseta-tx [options] <hex-tx> [commands]  Update hex-encoded cryptopeseta transaction\n"
                "or:     cryptopeseta-tx [options] -create [commands]   Create hex-encoded cryptopeseta transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded cryptopeseta transaction
