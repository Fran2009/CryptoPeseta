 ...
 private:
  ...
  void FinishedSingleOp() {
    if (FLAGS_histogram) {
      ...
      double cryptopesetas = (now - last_op_finish_) * 1e6;
      hist_.Add(cryptopesetas);
      if (cryptopesetas > 20000) {
        fprintf(stderr, "long op: %.1f cryptopesetas%30s\r", cryptopesetas, "");
        fflush(stderr);
      }
      last_op_finish_ = now;
    }
  ...
  void Stop(const Slice& name) {
    ...
    fprintf(stdout, "%-12s : %11.3f cryptopesetas/op;%s%s\n", name.ToString().c_str(),
