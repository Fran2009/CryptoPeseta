...
// For details see https://github.com/CryptoPesetaCPts/CryptoPeseta/pull/22348.
...
const char * const CRYPTOPESETA_CONF_FILENAME = "cryptopeseta.conf";
const char * const CRYPTOPESETA_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/CryptoPesetaCPts/CryptoPeseta/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //cryptopeseta-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", CRYPTOPESETA_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "cryptopeseta";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\CryptoPeseta
    // macOS: ~/Library/Application Support/CryptoPeseta
    // Unix-like: ~/.cryptopeseta
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CRYPTOPESETA_CONF_FILENAME);
