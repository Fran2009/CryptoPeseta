# Bootstrappable CryptoPeseta Core Builds

This directory contains the files necessary to perform bootstrappable CryptoPeseta
Core builds.
...
To build CryptoPeseta Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      CryptoPeseta Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/cryptopeseta-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/cryptopeseta-core/guix.sigs).
