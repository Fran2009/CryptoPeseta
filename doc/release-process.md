...
- Translations on Transifex
    - Create [a new resource](https://www.transifex.com/cryptopeseta/cryptopeseta/content/) named after the major version with the slug `[cryptopeseta.qt-translation-<RRR>x]`, where `RRR` is the major branch number padded with zeros. Use `src/qt/locale/cryptopeseta_en.xlf` to create it.
    ...
    - Make an announcement that translators can start translating for the new version. You can use one of the [previous announcements](https://www.transifex.com/cryptopeseta/cryptopeseta/announcements/) as a template.
    - Change the auto-update URL for the resource to `master`, e.g. `https://raw.githubusercontent.com/cryptopeseta/cryptopeseta/master/src/qt/locale/cryptopeseta_en.xlf`. (Do this only after the previous steps, to prevent an auto-update from interfering.)
...
To tag the version (or release candidate) in git, use the `make-tag.py` script from [cryptopeseta-maintainer-tools](https://github.com/cryptopeseta-core/cryptopeseta-maintainer-tools). From the root of the repository run:

    ../cryptopeseta-maintainer-tools/make-tag.py v(new version, e.g. 0.20.0)
...
Open a draft of the release notes for collaborative editing at https://github.com/cryptopeseta-core/cryptopeseta-devwiki/wiki.

For the period during which the notes are being edited on the wiki, the version on the branch should be wiped and replaced with a link to the wiki which should be used for all announcements until `-final`.

Generate the change log. As this is a huge amount of work to do manually, there is the `list-pulls` script to do a pre-sorting step based on github PR metadata. See the [documentation in the README.md](https://github.com/cryptopeseta-core/cryptopeseta-maintainer-tools/blob/master/README.md#list-pulls).
...
Checkout the CryptoPeseta Core version you'd like to build:
...
Add other builders keys to your gpg keyring, and/or refresh keys: See `../cryptopeseta/contrib/builder-keys/README.md`.
...
Codesigner only: Sign the macOS binary:
    ...
    transfer cryptopeseta-osx-unsigned.tar.gz to macOS for signing
    tar xf cryptopeseta-osx-unsigned.tar.gz
...
Codesigner only: Sign the windows binaries:
    ...
    tar xf cryptopeseta-win-unsigned.tar.gz
...
However if this is done, once the release has been tagged in the cryptopeseta-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds.

Codesigner only: Commit the detached codesign payloads:

```sh
pushd ./cryptopeseta-detached-sigs
...
- Detached signatures will then be committed to the [cryptopeseta-detached-sigs](https://github.com/cryptopeseta-core/cryptopeseta-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
- Create a torrent of the `/var/www/bin/cryptopeseta-core-${VERSION}` directory such
  that at the top level there is only one file: the `cryptopeseta-core-${VERSION}`
  directory containing everything else. Name the torrent
  `cryptopeseta-${VERSION}.torrent` (note that there is no `-core-` in this name).
...
- Update other repositories and websites for new version
  ...
  - cryptopeseta.com RPC documentation update
      ...
      - Install the new CryptoPeseta Core release
  ...
  - Update packaging repo
      ...
      - Push the flatpak to flathub, e.g. https://github.com/flathub/org.cryptopesetacore.cryptopeseta-qt/pull/2

      - Push the latest version to master (if applicable), e.g. https://github.com/cryptopeseta-core/packaging/pull/32

      - Create a new branch for the major release "0.xx" from master (used to build the snap package) and request the
        track (if applicable), e.g. https://forum.snapcraft.io/t/track-request-for-cryptopeseta-core-snap/10112/7

      - Notify MarcoFalke so that he can start building the snap package

        - https://code.launchpad.net/~cryptopeseta-core/cryptopeseta-core-snap/+git/packaging (Click "Import Now" to fetch the branch)
        - https://code.launchpad.net/~cryptopeseta-core/cryptopeseta-core-snap/+git/packaging/+ref/0.xx (Click "Create snap package")
        - Name it "cryptopeseta-core-snap-0.xx"
        - Leave owner and series as-is
        - Select architectures that are compiled via guix
        - Leave "automatically build when branch changes" unticked
        - Tick "automatically upload to store"
        - Put "cryptopeseta-core" in the registered store package name field
        - Tick the "edge" box
        ...
        - Promote release on https://snapcraft.io/cryptopeseta-core/releases if it passes sanity checks
