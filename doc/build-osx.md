...
This guide describes how to build cryptopesetad, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build CryptoPeseta Core from source.
...
### 4. Clone CryptoPeseta repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the CryptoPeseta Core repository to a directory.
...
CryptoPeseta Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the CryptoPeseta Core application using `make deploy`.
...
## Building CryptoPeseta Core

### 1. Configuration

There are many ways to configure CryptoPeseta Core, here are a few common examples:
...
Run the following in your terminal to compile CryptoPeseta Core:
...
## Running CryptoPeseta Core

CryptoPeseta Core should now be available at `./src/cryptopesetad`.
If you compiled support for the GUI, it should be available at `./src/qt/cryptopeseta-qt`.

The first time you run `cryptopesetad` or `cryptopeseta-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/CryptoPeseta/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/CryptoPeseta"

touch "/Users/${USER}/Library/Application Support/CryptoPeseta/cryptopeseta.conf"

chmod 600 "/Users/${USER}/Library/Application Support/CryptoPeseta/cryptopeseta.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/CryptoPeseta/debug.log
```

## Other commands:

```shell
./src/cryptopesetad -daemon      # Starts the cryptopeseta daemon.
./src/cryptopeseta-cli --help    # Outputs a list of command-line options.
./src/cryptopeseta-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/cryptopeseta-qt -server # Starts the cryptopeseta-qt server mode, allows cryptopeseta-cli control
```
