...
The CryptoPeseta Core daemon can be configured to act as a trusted "border
router", implementing the cryptopeseta wire protocol and relay, making
...
The ZeroMQ feature in CryptoPeseta Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building cryptopesetad:
...
For instance:
    ...
    $ cryptopesetad -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/cryptopesetad.tx.raw \
...
These options can also be provided in cryptopeseta.conf.
...
From the perspective of cryptopesetad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into cryptopesetad directly. Furthermore, no information is
...
using. CryptoPesetad appends an up-counting sequence number to each
